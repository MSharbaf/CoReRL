...
public class ResolvingModelSelectionPage extends WizardPage {
	public String path ; 
	...
	public FileChooser VersionMModelFC ;
	
	public IProject selectedProject;
	public ResolvingPreferenceSelectionPage ResolvingPreferenceSelectionPage ;

	public EmfModel bModel = null ; 
	public EmfModel modelV1 = null ; 
	public EmfModel modelV2 = null ; 
	public EmfModel modelVM = null ;
	
    protected ResolvingModelSelectionPage(String pageName) {
             super(pageName);
...
//		
		...
		VMMSection = new Group(container, SWT.NULL);
		VMMSection.setText("Select Target Model to Save Merged Version");
		VMMSection.setLayout(new GridLayout());
		VMMSection.setLayoutData(data);
		VersionMModelFC = new FileChooser(VMMSection) ; 
		VersionMModelFC.Extensions = "*.xmi; *.model; *.uml; *.ecore" ; 
		VersionMModelFC.ExtensionsName = "Model" ; 
		GridData VMMgridData = new GridData(GridData.FILL_HORIZONTAL) ; 
		VersionMModelFC.setLayoutData(VMMgridData);
	 ...
	 */
	 protected boolean nextPressed() {
		if(((this.MetaModelFC.path == null) && this.BaseModelFC.path.endsWith(".uml") == false && this.BaseModelFC.path.endsWith(".ecore") == false)
				|| (this.BaseModelFC.path == null) || this.Version1ModelFC.path == null || this.Version2ModelFC.path == null || this.VersionMModelFC.path == null){
		...
		{	
			...
			if((MetaModelFC.path == null && umlFlag==false && ecoreFlag==false) || BaseModelFC.path == null || Version1ModelFC.path == null || Version2ModelFC.path == null || VersionMModelFC.path == null){
				this.setDescription("You must select all the required files ...");
			}
			else
			{
				...
				if(this.BaseModelFC.path.endsWith(".uml")){
					try {
						...
						modelVM = getUmlModel("VersionM", new File(this.Version2ModelFC.path));
					} catch (Exception e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}		
					...
					try {
						...
						modelVM = getEcoreModel("VersionM", new File(this.VersionMModelFC.path)) ;
					} catch (Exception e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
					...
					try {
						...
						modelVM = getEmfModel("VersionM",this.VersionMModelFC.path, this.MetaModelFC.path, true, true);
				...
				modelVM.getAliases().add("Target");

				try { 
					if(this.ResolvingPreferenceSelectionPage != null)
						this.ResolvingPreferenceSelectionPage.onLoad(selectedProject, MetaModelFC, BaseModelFC, bModel, modelV1, modelV2, modelVM);
	...
	public void onLoad(IProject selectedPrj)
	{
		...
		VersionMModelFC.directory = this.path ; 
	}
